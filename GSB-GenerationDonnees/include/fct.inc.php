...
function majFicheFrais($pdo)
{
	...
	foreach($lesFichesFrais as $uneFicheFrais)
	{
		...
		$req = "select sum(montant) as cumul from ligneFraisHorsForfait where ligneFraisHorsForfait.idVisiteurFicheFrais = '$idVisiteur' 
		...
		ligneFraisForfait.idFraisForfait = fraisForfait.id   and   ligneFraisForfait.idVisiteurFicheFrais = '$idVisiteur' 
				and ligneFraisForfait.moisFicheFrais = '$mois' ";
		$res = $pdo->query($req);
		$ligne = $res->fetch();
